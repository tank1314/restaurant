			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="userSiteRelation/listUsers.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
                                    ...
                                    <th class="center">未关联门店</th>
                                    <th class="center">已关联门店</th>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
                                            ...
                                            <td class="center">
                                                <c:if test="${user.bus_name eq null }">
                                                    --
                                                </c:if>
                                                ${user.bus_name}
                                            </td>
                                            ...
                                            <td class="center">
                                                <c:if test="${QX.edit == 1 }">
                                                    <a style="cursor:pointer;" onclick="relationSite('${user.USER_ID}',1);" class="btn btn-warning btn-mini" data-rel="tooltip" title="未关联门店">
                                            ...
                                            <td class="center">
                                                <c:if test="${QX.edit == 1 }">
                                                    <a style="cursor:pointer;" onclick="relationSite('${user.USER_ID}',2);" class="btn btn-warning btn-mini" data-rel="tooltip" title="未关联门店">
                                                        <span class="red"> <i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></span>
                                                    </a>
                                                </c:if>
                                                <c:if test="${QX.edit != 1 }">
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>siteUser/goAddU.do?userType=2';
	 diag.Width = 469;
	 diag.Height = 510;
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>siteUser/goEditU.do?USER_ID='+user_id+"&userType=2";
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
//关联商铺
function relationSite(user_id,type){
    top.jzts();
    var diag = new top.Dialog();
    diag.Drag=true;
    if(1 == type){
    ...
    diag.URL = '<%=basePath%>userSiteRelation/goRelation.do?USER_ID='+user_id+"&type="+type;
    diag.Width = 860;
    diag.Height = 660;
    diag.CancelEvent = function(){ //关闭事件
        if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
