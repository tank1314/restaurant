...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.site_sid}</td>
											<td class='center'>${var.site_name}</td>
											<td class='center'>${var.catagory}</td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-sm btn-success" onclick="javaScript:void(0);">保存</a>
									</c:if>
	...
	<script type="text/javascript">
		...
		function goSondict(DICTIONARIES_ID){
			top.jzts();
			window.location.href="<%=path%>dictionaries/list.do?DICTIONARIES_ID="+DICTIONARIES_ID;
		};
		...
		function add(DICTIONARIES_ID){
			 ...
			 diag.CancelEvent = function(){ //关闭事件
				 if('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display){
					 parent.location.href="<%=path%>dictionaries/listAllDict.do?DICTIONARIES_ID=${DICTIONARIES_ID}&dnowPage=${page.currentPage}";
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=path%>dictionaries/delete.do?DICTIONARIES_ID="+Id+"&tm="+new Date().getTime();
