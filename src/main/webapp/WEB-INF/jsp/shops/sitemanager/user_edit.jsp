					...
					<div class="row">
						<div class="col-xs-12">
                            <form action="siteUser/${msg }.do" name="userForm" id="userForm" method="post">
                                <input type="hidden" name="bus_id" id="bus_id" value="${pd.bus_id}">
                                <input type="hidden" name="USER_ID" id="user_id" value="${pd.USER_ID }"/>
                                <div id="zhongxin" style="padding-top: 13px;">
                                    <c:if test="${pd.userType == 2}">
                                        <c:if test="${pd.isBusData == 0 }">
                                            <table id="table_report1" class="table table-striped table-bordered table-hover">
                                                用户：<b><font color="red">${pd.name}</font></b>，您好！</br>系统暂时未为您分配<u>商户</u>，请联系系统管理员分配商户。
                                            </table>
                                        </c:if>
                                        <c:if test="${pd.isBusData == 1 }">
                                            <table id="table_report" class="table table-striped table-bordered table-hover">
                                                ...
                                                <c:if test="${fx == 'head'}">
                                                    <input name="ROLE_ID" id="role_ids" value="${pd.ROLE_ID }" type="hidden" />
                                                </c:if>
                                                    <%--<tr>
