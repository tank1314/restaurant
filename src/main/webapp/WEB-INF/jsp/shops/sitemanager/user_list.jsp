			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="siteUser/listUsers.do" method="post" name="userForm" id="userForm">
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
                                            ...
                                            <td class="center">
                                                <c:if test="${QX.edit == 1 }">
                                                    <a style="cursor:pointer;" onclick="relationBus('${user.USER_ID}','${user.bus_id}');" class="btn btn-warning btn-mini" data-rel="tooltip" title="关联商户">
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>siteUser/goAddU.do?userType=1';
	 diag.Width = 469;
	 diag.Height = 510;
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>siteUser/goEditU.do?USER_ID='+user_id+"&userType=1";
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function relationBus(user_id,bus_id){
    ...
    diag.Drag=true;
    diag.Title ="关联商户";
    diag.URL = '<%=basePath%>siteUser/goRelation.do?USER_ID='+user_id+"&bus_id="+bus_id;
    diag.Width = 469;
    diag.Height = 400;
    diag.CancelEvent = function(){ //关闭事件
        if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
