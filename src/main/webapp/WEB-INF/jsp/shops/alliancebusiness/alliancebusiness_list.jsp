...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="alliancebusiness/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="bus_status" id="bus_status" data-placeholder="请选择加盟状态" style="vertical-align:top;width: 140px;">
									<option value=""></option>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.bus_name}</td>
											<td class='center'>${var.deposit_money}</td>
											<td class='center'>${var.belong_unit}</td>
											<%--<td class='center'>${var.bus_master}</td>--%>
											<td class='center'>
											    <c:if test="${var.bus_status ==0}">撤销</c:if>
                                                <c:if test="${var.bus_status ==1}">加盟中</c:if>
                                            </td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>alliancebusiness/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>alliancebusiness/delete.do?id="+Id+"&tm="+new Date().getTime();
        ...
        * @param type
         */
		function connectionInfo(id,name,type){
            top.jzts();
            var diag = new top.Dialog();
            diag.Drag=true;
            if("shop" == type){
                diag.Title ="关联门店";
                diag.URL = '<%=basePath%>alliancebusiness/goEdit.do?id='+Id;
            }
            if("user" == type)
            diag.Title ="关联加盟商";
            diag.URL = '<%=basePath%>alliancebusiness/goEdit.do?id='+Id;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>alliancebusiness/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
